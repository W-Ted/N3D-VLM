...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs) -> None:
        ...
        self.pcd_pe = PositionEmbeddingSine3D(embedding_size=1280, temperature=10000, n_points=1)
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, point_clouds: torch.Tensor) -> torch.Tensor:
        """
        Args:
        ...
        if True:
            with torch.no_grad():
                valid_mask = point_clouds[0, :, -1] > 0
                world_coords = point_clouds[0, :, :3] # Nx3
                if (~valid_mask).any():
                    valid_mean = world_coords[valid_mask].mean(dim=0)
                    world_coords[~valid_mask] = valid_mean
                patch_h, patch_w = grid_thw[0, 1], grid_thw[0, 2]
                if patch_h < patch_w:
                    world_coords = world_coords.reshape(1, -1, 640, 3)
                else:
                    world_coords = world_coords.reshape(1, 640, -1, 3)
            assert torch.isfinite(world_coords).all(), "Tensor contains NaN or Inf values"
            world_coords_avg = F.adaptive_avg_pool2d(
                world_coords.permute(0, 3, 1, 2).float(),
                output_size=(patch_h, patch_w)
            )  # 1 3 patch_h patch_w
            min_xyz = torch.mean(world_coords_avg.reshape(3, -1).mean(1))
            world_coords_avg = (world_coords_avg[0] - min_xyz) / 0.001 # 3 patc_h patch_w
            world_coords_avg = world_coords_avg.round()[None].permute(0, 2, 3, 1).flatten(1, 2)  # 1 3 patc_h*patch_w
            pe_feat = self.pcd_pe(world_coords_avg)
            hidden_states += pe_feat[0]
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None, point_clouds: Optional[torch.LongTensor] = None):
        """
        ...
        point_clouds = point_clouds.type(self.visual.dtype)
        image_embeds = self.visual(pixel_values, grid_thw=image_grid_thw, point_clouds=point_clouds)
        return image_embeds
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None: 
                image_embeds = self.get_image_features(pixel_values, image_grid_thw, point_clouds) # torch.Size([329/336/330/324, 3584]) torch.bfloat16
...
class Qwen2_5_VLForConditionalGeneration_pe(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        >>> import requests
        >>> from transformers import AutoProcessor, Qwen2_5_VLForConditionalGeneration_pe

        >>> model = Qwen2_5_VLForConditionalGeneration_pe.from_pretrained("Qwen/Qwen2.5-VL-7B-Instruct")
...
__all__ = ["Qwen2_5_VLForConditionalGeneration_pe", "Qwen2_5_VLModel", "Qwen2_5_VLPreTrainedModel", "Qwen2_5_VLTextModel"]
