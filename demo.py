...
for nn in trange(len(data)):
    ...
    cur_gt_list_2d = [xyz_to_2d_corners(bbox_cameraxy, depth_intr['intr']) for bbox_cameraxy in gt_list[0]]
    cur_pd_list_2d = [xyz_to_2d_corners(bbox_cameraxy, depth_intr['intr']) for bbox_cameraxy in pd_list[0]]

    cond = generate_scene_condition(
        question2=question2,
        pd_list=pd_list,
        cur_pd_list_2d=cur_pd_list_2d,
        auto_split=auto_split,
        round2_the_list=round2_the_list
    )
    
    if not auto_split:
        source_question = get_first_sentence(question2)
    else:
        source_question = get_first_sentence(question)

    if '<image>' not in source_question:
        source_question = '<image>'+source_question
    else:
        source_question = source_question

    if '<image>' in source_question:
        question2_update = source_question + cond
    else:
        question2_update = '<image>' + source_question + cond

    sample2 = {
            "messages": [
        {
            "content": question2_update,
            "role": "user"
        },
        {
            "content": answer2,
    ...
    encoded_pairs = template.encode_multiturn(tokenizer, messages2, None, None)
    total_length = len(input_ids) + (1 if template.efficient_eos else 0)
    ...
    save_dict.append(
            {
                ...
                "3D Spatial Reasoning": pred_answer2,
