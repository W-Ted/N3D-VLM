...
@dataclass
class Qwen2PointPlugin(BasePlugin):
    @override
    def _preprocess_image(self, image: "ImageObject", **kwargs) -> "ImageObject":
        image = super()._preprocess_image(image, **kwargs)
        if min(image.width, image.height) < 28:
    ...
    def _regularize_points(
        self, points: list["PointInput"], sampling_rate: float, is_eval=False, **kwargs
    ) -> dict[str, Union[list["NDArray"], list[float]]]:
        r"""Regularizes audios to avoid error. Including reading and resampling."""
        results = []
        for point in points:
            print(f'Convert points to np.array from {type(point)}')
            results.append(np.array(point))
        return results
...
PLUGINS = {
    ...
    "n3d_vlm": Qwen2PointPlugin, # 在线点云加载、图片加载
}
...
def get_mm_plugin_point(
    ...
    return PLUGINS[name](image_token, video_token, audio_token, point_token)